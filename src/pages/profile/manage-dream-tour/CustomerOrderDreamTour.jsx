...
function CustomerOrderDreamTour() {
  ...
  const { cusBookingId } = useParams();
  const navigate = useNavigate();

  const fetchOrder = async () => {
    try {
    const response = await api.get(`order/customerBooking-id/${cusBookingId}`);
  ...
  return (
    <div className="consul-viewOrder-container">
        <span
          ...
          onClick={() => navigate("/profile/dream-tour-booking")}
        >
          <Flex align="center">
            <IoIosArrowBack /> Back to your booking
          </Flex>
        </span>
    <Flex justify="center">
      <h1>Fish Order</h1>
...
export default CustomerOrderDreamTour
