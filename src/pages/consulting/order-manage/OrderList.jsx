...
function OrderList() {
  ...
  const fetchOrdersAndFish = async () => {
    try {
      ...
      const bookingId = ordersList[0].booking?.bookingId;
      ...
      message.error(
        err.response?.data?.message ||
          "Failed to fetch orders and available fish"
      );
    }
  };
              ...
              <div className="order-cancel-note">
                ...
                <div className="order-btn">
                  ...
                  {item.status === "DELIVERING" && (
                    <Button
                      className="btn-done-order"
                      onClick={() => updateOrderToDone(item.id)}
                    >
                      Done
                    </Button>
                  )}
