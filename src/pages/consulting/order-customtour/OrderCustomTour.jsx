...
import "./OrderCustomTour.css"
import api from "../../../config/axios";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import { toast } from "react-toastify";
import { Avatar, Button, Collapse, Flex, Form, InputNumber, List } from "antd";
function OrderCustomTour() {
  const [order, setOrder] = useState(null);
  const { customBookingId } = useParams();
  ...
  const fetchOrder = async () => {
    try {
      const response = await api.get(`order/customerBooking-id/${customBookingId}`);
  ...
  const fetchFarmFish = async () => {
    try {
      const response = await api.get(
        `koi/list-koi/custom-booking-id?customBookingId=${customBookingId}`
      );
  ...
  useEffect(() => {
    if (order && Object.keys(order).length > 0) {
      navigate(`/consulting-edit-order-customTour/${order.koiFishOrderId}`);
    }
  }, [order]);
  ...
  const onFinish = async (values) => {
    ...
    try {
      const reponse = await api.post("order/order-customer", orderInfo);
...
export default OrderCustomTour
