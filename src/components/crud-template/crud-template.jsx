...
function CRUDTemplate({ columns, formItems, path, afterAddItem }) {
  ...
  const fetchData = async (page, pageSize) => {
    try {
      const response = await api.get(
        `${path}/guest/get?page=${page - 1}&size=${pageSize}`
      );
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (dataToSend.image1 && dataToSend.image1.length > 0) {
        const file = dataToSend.image1[0];
        if (file.originFileObj) {
          dataToSend.image1 = await uploadFile(file.originFileObj);
        } else if (file.url) {
          dataToSend.image1 = file.url;
        } else {
          dataToSend.image1 = null;
        }
      } else {
        dataToSend.image1 = null;
      }

      if (dataToSend.image2 && dataToSend.image2.length > 0) {
        const file = dataToSend.image2[0];
        if (file.originFileObj) {
          dataToSend.image2 = await uploadFile(file.originFileObj);
        } else if (file.url) {
          dataToSend.image2 = file.url;
        } else {
          dataToSend.image2 = null;
        }
      } else {
        dataToSend.image2 = null;
      }
